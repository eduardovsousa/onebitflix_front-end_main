...
const UserForm = function() {
  ...
  const handleUserUpdate = async function (event: FormEvent<HTMLFormElement>) {
    ...
    if (res === 200) {
      setToastIsOpen(true);
      setErrorMessage("Informações alteradas com suceso!");
      setColor("bg-success");
      ...
      <Form onSubmit={handleUserUpdate} className={ styles.form }>
        ...
        <FormGroup>
          <Label for="lastName" className={ styles.label }>SOBRENOME</Label>
            <Input name="lastName" type="text" id="lastName" placeholder="Qual o seu último nome?" required maxLength={20} className={ styles.inputFlex } value={lastName} onChange={(event) => {setLastName(event.target.value)}}/>
        </FormGroup>
        </div>
        ...
        <FormGroup>
          <Label for="phone" className={ styles.label }>E-MAIL</Label>
            <Input name="email" type="email" id="email" placeholder="Informe o seu e-mail" required className={ styles.input } value={email} onChange={(event) => {setEmail(event.target.value)}}/>
        </FormGroup>
